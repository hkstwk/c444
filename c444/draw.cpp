...
void transposePlane(int direction, int delay){
	for (int step = 0; step < 5; step++){
		for (int z=0;z<CUBE_SIZE;z++)
		{
			if (direction == TRANSPOSE_X0Y0){
				cube[z][0] = transposeX0Y0[step][0];
				cube[z][1] = transposeX0Y0[step][1];
			}
			if (direction == TRANSPOSE_Y0Z0){
				cube[z][0] = transposeY0Z0[step][z][0];
				cube[z][1] = transposeY0Z0[step][z][1];
			}
			if (direction == TRANSPOSE_Z0X0){
				cube[z][0] = transposeZ0X0[step][z][0];
				cube[z][1] = transposeZ0X0[step][z][1];
			}
		}
		delay_ms(delay);
		fill(0x00);
